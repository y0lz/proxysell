...
try {
    db.exec(`ALTER TABLE users ADD COLUMN plus_until TEXT`);
}
catch { /* уже есть */ }
try {
    db.exec(`ALTER TABLE users ADD COLUMN sub_notified INTEGER NOT NULL DEFAULT 0`);
}
catch { /* уже есть */ }
try {
    db.exec(`ALTER TABLE users ADD COLUMN reroll_count INTEGER NOT NULL DEFAULT 0`);
}
catch { /* уже есть */ }
try {
    db.exec(`ALTER TABLE users ADD COLUMN reroll_window_start TEXT`);
}
catch { /* уже есть */ }
try {
    db.exec(`ALTER TABLE users ADD COLUMN last_reroll_at TEXT`);
}
catch { /* уже есть */ }
try {
    db.exec(`ALTER TABLE users ADD COLUMN shown_proxy_ids TEXT NOT NULL DEFAULT '[]'`);
}
catch { /* уже есть */ }
...
const stmtGetUsersForNotification = db.prepare(`
  SELECT * FROM users
...
`);
export const users = {
    upsert: (id, username, first_name) => stmtUpsertUser.run({ id, username: username ?? null, first_name: first_name ?? null }),
    get: (id) => stmtGetUser.get(id),
    // Истечение подписок
    expireSubscriptions: () => stmtExpireSubscriptions.run(),
    // Уведомления о подписке
    getUsersForNotification: () => stmtGetUsersForNotification.all(),
    setSubNotified: (id) => stmtSetSubNotified.run(id),
    resetSubNotified: (id) => stmtResetSubNotified.run(id),
    ...
    updateRerollData: (userId, rerollCount, windowStart, lastRerollAt, shownProxyIds) => {
...
// ─── Plans ────────────────────────────────────────────────────────────────────
const stmtGetAllPlans = db.prepare(`SELECT * FROM plans WHERE id != 'free' ORDER BY stars ASC`);
const stmtGetPlanById = db.prepare(`SELECT * FROM plans WHERE id = ?`);
export const plans = {
    getAll: () => stmtGetAllPlans.all(),
    getById: (id) => stmtGetPlanById.get(id),
};
// ─── Payments ─────────────────────────────────────────────────────────────────
...
`);
const stmtGetPaymentById = db.prepare(`
  SELECT * FROM payments WHERE id = ?
`);
const stmtGetPaymentByPayload = db.prepare(`
...
const stmtUpdatePaymentStatus = db.prepare(`
  ...
  WHERE id = @id
`);
const stmtGetUserPayments = db.prepare(`
  SELECT * FROM payments WHERE user_id = ? ORDER BY created_at DESC
`);
// Подтверждение оплаты — транзакция: обновить payment + продлить подписку
const confirmPayment = db.transaction((paymentId) => {
...
export const payments = {
    create: (user_id, plan, duration_days, stars_amount, payload) => {
    ...
    getById: (id) => stmtGetPaymentById.get(id),
    getByPayload: (payload) => stmtGetPaymentByPayload.get(payload),
    updateStatus: (id, status) => stmtUpdatePaymentStatus.run({ id, status }),
    getUserPayments: (user_id) => stmtGetUserPayments.all(user_id),
...
`);
const stmtGetFastActive = db.prepare(`
  SELECT * FROM proxies
  WHERE status = 'active'
  ORDER BY ping_ms ASC
  LIMIT 1
`);
const stmtGetFastActiveByType = db.prepare(`
  SELECT * FROM proxies
...
const stmtGetVipProxy = db.prepare(`
  SELECT * FROM proxies
  WHERE status = 'active' AND type = ?
...
const stmtGetProxyForNewUser = db.prepare(`
  SELECT * FROM proxies
...
const stmtGetPlusProxy = db.prepare(`
  SELECT * FROM proxies
  WHERE status = 'active' AND type = ?
  ORDER BY likes DESC, ping_ms ASC
  LIMIT 1
...
const stmtGetFreeProxy = db.prepare(`
  SELECT * FROM proxies
  WHERE status = 'active' AND type = ?
  ORDER BY RANDOM()
  LIMIT 1
...
function getNextProxyWithHistory(type, shownIds, isPlus) {
    let query;
    let params;
...
function canReroll(userId) {
    const user = stmtGetUser.get(userId);
    if (!user)
        return { allowed: false, reason: 'ok' };
    const now = new Date();
...
const stmtGetUnchecked = db.prepare(`
...
`);
const stmtGetById = db.prepare(`SELECT * FROM proxies WHERE id = ?`);
...
export const proxies = {
    insert: (type, link) => stmtInsertProxy.run({ type, link, status: "unchecked" }),
    getFastActive: () => stmtGetFastActive.get(),
    getFastActiveByType: (type) => stmtGetFastActiveByType.get(type),
    // Прокси для новых пользователей (средний по рейтингу)
    getProxyForNewUser: () => stmtGetProxyForNewUser.get(),
    // Получить следующий прокси с учётом shown_proxy_ids и инфинити-ролла
    getNextProxy: (userId, type = "MTPROTO") => {
        const user = stmtGetUser.get(userId);
        if (!user)
            return undefined;
        const isPlus = users.isPlus(user);
    ...
    // Проверка возможности реролла
    canReroll: (userId) => canReroll(userId),
    // Записать реролл
    recordReroll: (userId) => recordReroll(userId),
    getById: (id) => stmtGetById.get(id),
    getUnchecked: () => stmtGetUnchecked.all(),
    setStatus: (id, status, ping_ms) => stmtSetStatus.run({ id, status, ping_ms }),
    setCountry: (id, country) => stmtSetCountry.run({ id, country }),
    vote: (proxy_id, user_id, vote) => {
    ...
    countByStatus: (status) => stmtCountByStatus.get(status)?.count ?? 0,
};
export default db;
