...
async function reportWithRetry(results) {
    for (let i = 0; i < 3; i++) {
        try {
...
// ─── Проверки ─────────────────────────────────────────────────────────────────
function checkSocks5(host, port) {
    return new Promise((resolve, reject) => {
        ...
        const req = https.get({ hostname: "api.telegram.org", port: 443, path: "/", agent }, (res) => {
            if (done)
                return;
            done = true;
            clearTimeout(timer);
...
function checkMtproto(host, port) {
    return new Promise((resolve, reject) => {
        const start = Date.now();
...
async function runCycle() {
    ...
    let stats;
    try {
        stats = await apiRequest("GET", "/stats");
    }
    catch (err) {
    ...
    while (true) {
        let batch;
        try {
            batch = await apiRequest("GET", "/unchecked");
        }
        catch (err) {
...
http.createServer((req, res) => {
    ...
    if (req.method === "POST" && req.url === "/restart-peer") {
        ...
        const req2 = http.request({ hostname: url.hostname, port: url.port, path: "/restart", method: "POST",
            headers: { "x-api-secret": API_SECRET }, timeout: 5_000 }, (r) => { r.resume(); });
        req2.on("error", () => { });
        req2.end();
        return;
    }
...
//# sourceMappingURL=checker-agent.js.map